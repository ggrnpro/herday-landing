...
export function EmailModal({ payload, onClose }: Props) {
  ...
  const valid = !submitting && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
  ...
  async function submit() {
    ...
    setSubmitError(null);
    try {
      const res = await fetch("/api/inner-critic/email", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          email,
          name: payload.answers.name,
          critic: payload.answers.critic,
        ...
        <div className="p-8 md:p-12">
          {!sent ? (
            <>
              ...
              >
                Save these four to{" "}
                <em className="italic font-light text-merlot">your inbox</em>?
              </h2>
              <p className="mt-4 text-[15px] text-ink-soft max-w-[48ch]">
                One email, once. No newsletter, no follow-ups. So you have it
                somewhere on a Tuesday when the voice gets loud again.
              </p>
